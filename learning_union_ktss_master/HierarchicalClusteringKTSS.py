...
import numpy as np
from learning_union_ktss_master.KTestable import *
...
def ktssDistance(a,b,k_window):
    ...
    E1,I1,F1,T1,C1 = calculateEIFTC([a],k)
    E2,I2,F2,T2,C2 = calculateEIFTC([b],k)
...
def learn_unions_ktss(dataset,num_clusters,k_window,show_plot,crossover_detect,PATH_PLOT):
    ...
    for cluster in dict_clusters:
        E,I,F,T,C = calculateEIFTC(dict_clusters[cluster],k_window)
        kss = k_testable(E,I,F,T,C)
        dict_ktss[kss] = None
    return list(clusters), dict_ktss
